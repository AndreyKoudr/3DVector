...
public:
  ...
  TVector operator/(T divider) const {
    TVector v3;
    assert(std::abs(divider) > TOLERANCE(T) && "Division by zero in TVector");
    v3.X = X/divider;
    v3.Y = Y/divider;
    v3.Z = Z/divider;
    v3.W = W/divider;
  ...
  TVector operator/=(TVector divider) {
    assert((std::abs(divider.X) > TOLERANCE(T) && std::abs(divider.Y) > TOLERANCE(T) &&
      std::abs(divider.Z) > TOLERANCE(T) && std::abs(divider.W) > TOLERANCE(T)) && "Division by zero in TVector");
    X /= divider.X;
    Y /= divider.Y;
    Z /= divider.Z;
    W /= divider.W;
